import { BookOpen, Bug, Globe } from "lucide-react";
import { LogoMark } from "@/components/logo";

// lucide dropped brand icons, so the octicon mark ships locally
function GithubIcon({ className }: { className?: string }) {
  return (
    <svg
      viewBox="0 0 16 16"
      fill="currentColor"
      aria-hidden="true"
      className={className}
    >
      <path d="M8 0c4.42 0 8 3.58 8 8a8.013 8.013 0 0 1-5.45 7.59c-.4.08-.55-.17-.55-.38 0-.27.01-1.13.01-2.2 0-.75-.25-1.23-.54-1.48 1.78-.2 3.65-.88 3.65-3.95 0-.88-.31-1.59-.82-2.15.08-.2.36-1.02-.08-2.12 0 0-.67-.22-2.2.82-.64-.18-1.32-.27-2-.27-.68 0-1.36.09-2 .27-1.53-1.03-2.2-.82-2.2-.82-.44 1.1-.16 1.92-.08 2.12-.51.56-.82 1.28-.82 2.15 0 3.06 1.86 3.75 3.64 3.95-.23.2-.44.55-.51 1.07-.46.21-1.61.55-2.33-.66-.15-.24-.6-.83-1.23-.82-.67.01-.27.38.01.53.34.19.73.9.82 1.13.16.45.68 1.31 2.69.94 0 .67.01 1.3.01 1.49 0 .21-.15.45-.55.38A7.995 7.995 0 0 1 0 8c0-4.42 3.58-8 8-8Z" />
    </svg>
  );
}

const LINKS = [
  {
    label: "GitHub",
    href: "https://github.com/soroscan-io/soroscan",
    icon: GithubIcon,
  },
  {
    label: "Report an issue",
    href: "https://github.com/soroscan-io/soroscan/issues/new/choose",
    icon: Bug,
  },
  {
    label: "Stellar docs",
    href: "https://developers.stellar.org",
    icon: BookOpen,
  },
  { label: "stellar.org", href: "https://stellar.org", icon: Globe },
];

export function SiteFooter() {
  return (
    <footer className="pt-6">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-8 px-4 py-10 sm:px-6 md:flex-row md:items-start md:justify-between lg:px-8">
        <div className="flex max-w-xs flex-col gap-3">
          <span className="flex items-center gap-2">
            <LogoMark size={18} />
            <span className="-translate-y-[1px] font-semibold tracking-tight">
              soroscan
            </span>
          </span>
          <p className="text-muted-foreground">
            See what's happening on Stellar. Open source under MIT; data from
            public Horizon and Stellar RPC providers.
          </p>
        </div>
        <nav aria-label="Footer" className="grid grid-cols-2 gap-x-16 gap-y-4">
          {LINKS.map((link) => (
            <a
              key={link.href}
              href={link.href}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 text-muted-foreground transition-colors hover:text-foreground"
            >
              <link.icon className="size-4" aria-hidden="true" />
              {link.label}
            </a>
          ))}
        </nav>
      </div>
    </footer>
  );
}
